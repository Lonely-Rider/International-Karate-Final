...
;TO DO LIST: 			- toimiiko demossa kent‰n vaihto? 
;						- Sel‰lle/mahalle kaatuminen reunassa. 
;
;6.3.16 Kent‰n vaihto korjattu. Tiles -kentt‰ joka kent‰n v‰liss‰. Highscore.
;5.3.16 Slow Mo
;3.3.16  Toggle Music
;27.2.16 Muokattu Fistist‰ hits_fist.bb
;IK1-2, 26.11.2008
...
Global x_reuna_a = 750 ;cpu-ukkojen reunaan osuminen
Global x_reuna_b = 50 ;
...
Global slomo = 0 ; hidastuskoodi
;---- tiles - muuttujat
Global tf = 0
Global rf = 0
Global naputus = MilliSecs()
Global nappu = 0
Global nappuli = 0
Global napsis = 0
Global naputus2 = MilliSecs()
Global nappis = 0
Global napunapu
Global tiles = LoadAnimImage("graphics\tiles.png",110,140,0,13)
MaskImage tiles,1,1,255
Global tiles2 = LoadAnimImage("graphics\tiles2.png",110,140,0,13)
MaskImage tiles2,1,1,255
Global ready = LoadAnimImage("graphics\ready_set_go.png",110,35,0,3)
MaskImage ready,91,70,1
Global lento = 0
Global ilmaan = 2
Global ukon_esto = MilliSecs()
Global pelin_hidastus = MilliSecs()
Global	tf2 = 0
Global	bonusosumat = 0
Global	lentopisteet = 0
;___________
...
Global fontti = LoadFont("times new roman",18,True,False,False)
SetFont fontti
...
While (lopetus = 0)
	...
	EndIf
		...
		If siirr <> 4 Then		;tiles kenttien v‰liss‰
			THE_MAIN_LOOP()
		Else
			tiles()
		EndIf
...
;			PlaySound(musa2)
;		EndIf
		If (Not ChannelPlaying(musa)) Then PlayMusic("karate2.mp3")
		If (seur = 1) Then Exit
        Wend
...
x = 100 : x3 = 300: x2 = 580: y = 440 : y2 = 440 : y3 = 437
        ...
        vesilumi = Rand(0,1)
        frame_liekki = 0
        Local liekki = LoadAnimImage("graphics\liekki.png",30,24,0,3)
...
Function ajastus()
Text 0,210,"lights: "+lights+"  siirr:" + siirr

	  	;lis‰tty musan volumen s‰‰tˆ 
		If KeyDown(50) Then
			If volyymi = 1 Then
				ChannelVolume musa,.0
				volyymi = 0 : Delay 100 : FlushKeys()
			Else
				ChannelVolume musa,.9
				volyymi = 1 : Delay 100 : FlushKeys()			
			EndIf
		EndIf
...
Function lopputekstit()
		...
		credit$(54)  =  "   Lonely Rider Software (tm) "
		credit$(55)  =  "         2008  - 2016         "            
		
		Color 0,255,0
		loparit = LoadFont("times new roman", 24, True, False, False)
		SetFont loparit
...
Else
	...
	If (boonus = 0) Then
		...
		If (cpu_vai_ei < 9) Then ;demomode	
			...
			valot()	 			;valot syttyy osumasta, sijaitsee myˆs hits...bb:ss‰
		Else
...
Function slowmo()
	...
	If (KeyDown(49)) Then 
		If slomo = 1 Then
			slomo = 0 : Delay 100 : FlushKeys()
		Else
			slomo = 1 : Delay 100 : FlushKeys()	
		EndIf
	EndIf

	If (slomo = 1) Then Text 200,400,"Slow motion..." : Delay 250
