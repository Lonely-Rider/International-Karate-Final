		...
		If ((x < x3) And (suunta = 0) And isku_frame <> 1 And isku_frame < 13 Or isku_frame > 14) Then
			If (((isku3 = 0) And (ImagesCollide (iskuja,x,y,isku_frame,ukko3,x3,y,ukko3_frame))) Or ((isku3 = 1) And (ImagesCollide (iskuja,x,y,isku_frame,iskuja3,x3,y,isku3_frame)))) Then
				If ((x3 - x) < osu_xh) Then
					...
					If (x3 < 100 Or x3 > 700) Then  ;vihreä ukko, jos liian reunassa niin kaatuu istumaan
							osuma3_frame = 0
							osuma3_aloitus = 0
					EndIf
...
Function nollaus()
		...
		If (level = 4 Or level = 5 Or level = 6) Then
			;stars = LoadAnimImage("bubbles.bmp",50,20,0,3)
			;MaskImage stars,0,0,255
		Else
			;stars = LoadAnimImage("stars.bmp",50,20,0,3)
			;MaskImage stars,0,0,255
		EndIf
End Function
